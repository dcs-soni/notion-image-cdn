...
export async function cacheRoutes(fastify: FastifyInstance) {
  ...
  const hasApiKeys = config.apiKeysSet.size > 0;
  ...
  fastify.delete(
    '/api/v1/cache',
    {
      ...
      preHandler: hasApiKeys ? [authenticate] : [denyNoKeysConfigured],
    },
...
async function denyNoKeysConfigured(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  request.log.warn(
    { requestId: request.requestId },
    'Destructive endpoint accessed but no API keys are configured — rejecting',
  );
  reply.status(403).send({
    error: {
      ...
      message:
        'This endpoint requires API key authentication, but no API keys are configured. ' +
        'Set the API_KEYS environment variable to enable access.',
      requestId: request.requestId,
    },
  });
}
